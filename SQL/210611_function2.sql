...
select count(*) as "사원의 수", 
       ...
       max(sal) as "최대 급여",
       min(sal) as "최소 급여",
       sum(comm) as "커미션의 총합", -- 그룹 함수는 null 값을 제외하고 연산
       avg(comm) as "커미션의 평균"
from emp;
...
select deptno, count(*) as "부서인원", sum(sal) as "급여 총합", avg(sal) as "급여 평균", max(sal) as "최대 급여", min(sal) as "최소 급여"
from emp
group by deptno
order by deptno
;
...
-- group by의 그룹 함수의 결과를 비교 시에는 having 절을 사용
-- select 
-- from
-- where
-- group by
