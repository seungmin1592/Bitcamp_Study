...
select substr(hiredate, 3, 4) as "check"
from emp
where check = 4
;
...
-- 19. 입사일을 년도는 2자리(YY), 월은 숫자(MM)로 
--     표시하고 요일은 약어 (DY)로 지정하여 출력하시오.
select to_char(hiredate,'YY.MM.DY') as "입사년도"
from emp;

-- 20. 올해 몇 칠이 지났는지 출력하시오. 
--     현재날짜에서 올해 1월 1일을 뺀 결과를 출력하고
--     TO_DATE 함수를 사용하여 데이터 형을 일치 시키시오.
select to_date('2021.01.01', 'YYYY.MM.DD') as "오늘 날짜",
       floor(sysdate-to_date('2021.01.01', 'YYYY.MM.DD'))
from dual
;
...
select max(sal) as "최고 급여" , min(sal) as "최저 급여", max(sal) - min(sal) as "차액"
from emp
;
